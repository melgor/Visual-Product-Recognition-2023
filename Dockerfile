...
COPY --chown=1001:1001 MCS2023_baseline ${HOME_DIR}/MCS2023_baseline
COPY --chown=1001:1001 my_submission ${HOME_DIR}/my_submission
COPY --chown=1001:1001 visual_search ${HOME_DIR}/visual_search
COPY --chown=1001:1001 aicrowd.json ${HOME_DIR}/aicrowd.json
COPY --chown=1001:1001 apt.txt ${HOME_DIR}/apt.txt
COPY --chown=1001:1001 local_evaluation.py ${HOME_DIR}/local_evaluation.py
COPY --chown=1001:1001 mean_average_precision.py ${HOME_DIR}/mean_average_precision.py
COPY --chown=1001:1001 config ${HOME_DIR}/config
